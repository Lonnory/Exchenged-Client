...
import com.exchenged.client.dto.TrojanConfig
import com.exchenged.client.model.OutboundObject
import com.exchenged.client.model.TrojanOutboundConfigurationObject
import com.exchenged.client.model.TrojanServerObject
    ...
    override fun parseOutbound(url: String): OutboundObject<TrojanOutboundConfigurationObject> {
        ...
        return OutboundObject(
            ...
            streamSettings = StreamSettingsObject(
                ...
                tlsSettings = if (trojanConfig.params["security"] == "tls") {
                    TlsSettings(
                        serverName = trojanConfig.params["sni"]?.ifBlank { trojanConfig.params["peer"] }?.ifBlank { trojanConfig.host }
                            ?: trojanConfig.params["peer"] ?: trojanConfig.host,
                        allowInsecure = trojanConfig.params["allowInsecure"] == "1" || trojanConfig.params["allowInsecure"] == "true"
                    )
                } else null,
                realitySettings = if (trojanConfig.params["security"] == "reality") {
                    RealitySettings(
                        fingerprint = trojanConfig.params["fp"]?.ifBlank { "chrome" } ?: "chrome",
                        publicKey = trojanConfig.params["pbk"] ?: "",
                        serverName = trojanConfig.params["sni"]?.ifBlank { trojanConfig.params["peer"] }?.ifBlank { trojanConfig.host }
                            ?: trojanConfig.params["peer"] ?: trojanConfig.host,
                        shortIds = trojanConfig.params["sid"]?.let { if (it.isBlank()) null else listOf(it) },
                        spiderX = trojanConfig.params["spx"] ?: "",
                        show = false
                    )
                } else null,
                wsSettings = if (network == "ws") {
                    WsSettings(
                        ...
                        headers = mapOf(Pair("Host", trojanConfig.params["host"]?.ifBlank { trojanConfig.host } ?: trojanConfig.host ?: ""))
                    )
                } else null,
