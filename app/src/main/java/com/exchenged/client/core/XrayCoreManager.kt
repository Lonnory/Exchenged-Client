...
@Inject constructor(
    ...
    private val consumes: MutableList<Consumer<Pair<Double, Double>>> = CopyOnWriteArrayList()
    ...
    suspend fun startXrayCore(link: String, protocol: String, tunFd: Int?) {
        startOrClose = true
        try {
            val validFd = if (tunFd != null && tunFd > 0) tunFd else -1
            val includeTun = validFd > 0
            val config = parserFactory.getParser(protocol).parse(link, includeTun)
            coroutineScope.launch(Dispatchers.IO) {
                // Pass 0 if we don't have a valid FD to avoid JNI crashes in some core versions
                coreController?.startLoop(config, if (validFd > 0) validFd else 0)
            }
        }catch (e: Exception) {
    ...
    override fun startTrafficDetection() {
            job = coroutineScope.launch(Dispatchers.IO) {
                ...
                while (true) {
                    ...
                    last = cur
                    delay(1000) // Added delay to prevent high CPU usage in producer loop
            }
        }
    }
